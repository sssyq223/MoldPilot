...
SKILLS.update({'delivery_risk_analysis':{'name':'供应商发货风险分析','tools':['analyze_delivery_risk']},
               ...
               'full_outsource_review':{'name':'整套委外协同上下文核对','tools':['query_full_outsource_context'],
                   'optional_tools':['prepare_contract_signing_record','prepare_supplier_material_handoff','prepare_supplier_deduction_settlement'],
                   'activation_queries':['整套委外执行','整套委外加工','供应商节点','委外验收','委外扣款','委外合同签署','签署扫描件','资料交接','供应商资料']},
...
CAPABILITY_DEPARTMENTS = {
    ...
    'prepare_supplier_material_handoff': 'purchase',
...
CAPABILITY_TYPES = {
    ...
    'prepare_supplier_material_handoff': 'operation',
...
def tool_schema(key):
    ...
    if key=='query_delivery_logistics_context':
        from .plan_tools import ProjectPlanContextInput
        return {'type':'function','function':{'name':key,'description':TOOLS[key]['description'],'parameters':ProjectPlanContextInput.model_json_schema()}}
    if key in {'query_full_outsource_context','prepare_supplier_material_handoff'}:
        if key=='prepare_supplier_material_handoff':
            from .full_outsource_tools import supplier_material_handoff_schema
    ...
    if key in {'prepare_customer_receipt_confirmation','prepare_supplier_payment_confirmation','prepare_supplier_deduction_settlement'}:
        from .finance_context_tools import customer_receipt_schema, supplier_deduction_settlement_schema, supplier_payment_confirmation_schema
...
def execute(db, user, key, arguments, run=None):
    ...
    if key=='prepare_supplier_material_handoff':
        from .full_outsource_tools import execute_full_outsource_tool
