...
查询工具只读；prepare_contact_ 和 prepare_project_ 工具仅准备操作建议，返回 proposal 后等待用户在会话卡片中核对确认，不代表业务已执行。项目暂停/恢复建议经本人确认后也只是提交 Agent BPM，须把“已提交审批”和“审批已生效”明确区分。不得把自然语言同意当作确认凭证。用户仅查询时不得准备写入建议；用户要求办理时，查询真实对象标识、当前版本和可选流程，必要时追问，再准备对应建议。
...
def run_loop(context, model, gateway, max_turns=12, max_tools=30, max_seconds=300):
    ...
    while True:
        ...
        model_ok = False
        try:
            message = model.generate(messages, tools)
            model_ok = True
        finally:
