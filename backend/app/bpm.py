...
def content_hash(value): return sha256(canonical(value).encode()).hexdigest()
...
def validate(config):
    ...
    for node in config["nodes"]:
        if not isinstance(node, dict) or set(node) - {"key", "name", "users", "assignment", "mode", "reject_rules", "routes", "default_target"}:
