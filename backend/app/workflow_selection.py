...
def require_template(db, user, resource, definition_id):
    ...
    if definition.config.get('material_contract') is not None:
        # The current submit endpoints have no persisted, confirmed material binding yet.
        # Never substitute client simulation values or the old purchase snapshot for it.
        raise DomainError('MATERIALS_NOT_BOUND','该流程需要已核对的资料模板数据；资料导入与确认尚未接入，当前不能正式发起',409)
    return definition
