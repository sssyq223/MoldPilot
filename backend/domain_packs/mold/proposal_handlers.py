...
HANDLERS = (
    ProposalHandler("contact.execute", "app.contact_tools", frozenset({
    ...
    ProposalHandler("project_control.execute", "app.project_control_tools", frozenset({
        "prepare_project_pause", "prepare_project_resume",
    })),
    ProposalHandler("project_closure.execute", "app.project_closure_tools", frozenset({
    ...
    ProposalHandler("project_plan.execute", "app.plan_tools", frozenset({
        "prepare_project_plan_baseline", "prepare_project_plan_change",
        "prepare_plan_department_confirmation",
    })),
    ProposalHandler("internal_start.execute", "app.start_tools", frozenset({"prepare_internal_start"})),
    ProposalHandler("quote_acceptance.execute", "app.quote_tools", frozenset({"prepare_quote_acceptance_decision"})),
    ProposalHandler("contract.execute", "app.contract_tools", frozenset({
