目标：当本轮只有一份受支持的 XLSX、XLS 或 CSV 附件，且用户明确说“导入料单”“解析料单”或上传新模设计清单时，直接使用 `erp_design_parse_new_mold_upload`，不要再次追问附件是否为料单。默认省略 `sheet_type` 或使用 `auto`，由 D 盘 ERP 复用其现有表头解析规则自动识别钢料或五金；只有附件不明确、存在多个候选文件、格式不支持，或用户明确指定类型时，才追问或传 `steel`/`hardware`。该工具只建立 ERP 上传会话，不代表图纸处理完成。
...
导入前如果缺少交期、请购原因、紧急程度、模具号、清单类型或其他 ERP 要求的业务字段，必须发起追问并等待用户选择或补充，不能按行数、当前日期、历史对话或模型常识代填。交期尤其不得自动推算；没有明确交期时只能输出澄清问题，不得调用导入工具。ERP 或清单解析返回任何自动规范、自动修正、图纸回填、数量/料型/尺寸/价格调整时，必须逐行展示修正前后值和修正原因，先让用户核对，再请求导入确认；不得只显示“已自动处理”或把修正后的结果隐去。
