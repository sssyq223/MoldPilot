...
- 使用 `page_num`、`page_size` 分页，默认第一页、每页 10 条（上限 500 条）。分页和排序均由 ERP 的 `/design/group-keyword/list` 执行；根据 ERP 的 `total`、`hasNext` 说明本次返回范围，需要继续读取时再查询下一页，不把一页说成全部。
- 完整 ERP 回执由对话中的只读表格展示，字段为 NO.、关键词、备注、创建时间和更新时间；真实记录 ID 保留在 ERP 回执中但不在表格中展示。回答简述匹配总数、返回条数及查询时间即可，无需重复抄写完整表格。
