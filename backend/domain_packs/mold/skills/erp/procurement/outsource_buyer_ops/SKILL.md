...
1. 先用 `query_erp_outsource_followup_board`（或已有查询结果）锁定单据。同一模具多单时列出候选的订单号、模具号、批次号，等用户选定后再 prepare。尚未下单时用单一批次号；合并多批次的询价要用表格里的完整批次号。禁止使用内部数字 id。
