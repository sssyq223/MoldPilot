...
对象清楚就直接查，不必先复述。「收料待办 / 确认来料 / 确认收料」本轮必须 `CALL_TOOL` `query_erp_outsource_processor_fulfillment`，禁止 `CONVERSATION`。只问数量时不要 prepare。按 `nextAction`：收货才 prepare，等待仓库只说明。
