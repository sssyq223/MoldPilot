...
调用 `prepare_quote_acceptance_decision` 必须使用查询返回的真实 `project.id`、`project.row_version` 和流程 ID。若存在当前生效的结构化报价版本，还必须引用其业务记录 ID，承接金额和币种必须与该版本一致；不能绕过报价版本另填金额。承接时必须确认最终加工方式 `INTERNAL` 或 `FULL_OUTSOURCE`；拒单时不得填写加工方式。不得凭自然语言、截图或历史对话构造项目或流程。
