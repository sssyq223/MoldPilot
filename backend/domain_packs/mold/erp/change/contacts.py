...
from domain_packs.mold import models as m, authorization as auth
...
def serialize(db,c,details=False,user=None):
    ...
    if details:
        from domain_packs.mold.erp.change.contact_lifecycle import context,record_detail
...
def replay(db,user,c,data,action):
    ...
    if old:
        ...
        return digest,True
    from domain_packs.mold.erp.change.contact_lifecycle import ensure_open
    ensure_open(c)
