...
export function capabilityName(value:any,_items:any[]=[]){return typeof value==='string'?value:String(value?.name||value?.key||'能力')}
...
export function routeName(target:string,_nodes:any[]=[]){return target}
export function scopeText(scope:any,_resources:any[]=[]){return JSON.stringify(scope)}
