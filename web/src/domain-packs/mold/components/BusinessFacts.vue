...
function stageObjectText(key:string,value:any){
 ...
 if(key==='current_effective')return [value?.quotation_number,value?.version&&`V${value.version}`,value?.quoted_amount&&`${value.quoted_amount} ${value.currency||''}`,value?.preliminary_execution_mode&&valueText('execution_mode',value.preliminary_execution_mode)].filter(Boolean).join(' · ')||'—'
 if(key==='customer_start_conditions')return [
  `工艺方案${value?.customer_process_confirmed?'已确认':'未确认'}`,
  `外部订单${value?.external_order_number||'未记录'}`,
  `客户开工${value?.external_start_date||'未记录'}`,
  `客户交期${value?.customer_due_date||'未记录'}`,
  `开工通知附件${value?.has_external_start_notice?'已核对':'未见'}`,
 ].join('、')
...
function stageNextTool(stage:any){return stage?.action_tool||stage?.remediation_tool||stage?.query_tool}
