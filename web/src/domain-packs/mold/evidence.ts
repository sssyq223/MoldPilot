...
export function hasBusinessFactHighlights(row:any){const analysis=row?.analysis;return Boolean(analysis?.kickoff_lifecycle||analysis?.execution_lifecycle||analysis?.completion_lifecycle||analysis?.tasks?.length||analysis?.revision_impact||analysis?.plan_change_candidates?.length)}
