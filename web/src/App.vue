...
const noticeCount=computed(()=>new Set([...approvals.value.map(a=>'approval:'+a.id),...(approvalWorkItems.value.copied||[]).map((a:any)=>'approval:'+a.id),...(approvalWorkItems.value.overdue||[]).map((a:any)=>'approval:'+a.id),...notices.value.filter(n=>!n.read).map(n=>n.kind?.startsWith('approval.')?'approval:'+n.resource_id:'notice:'+n.id)]).size)
...
async function refresh(){ [conversations.value,notices.value,approvals.value,approvalWorkItems.value,capabilities.value]=await Promise.all([api('/conversations'),api('/notifications'),api('/approvals'),api('/approval-work-items'),api('/capabilities')]) }
