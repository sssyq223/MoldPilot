...
const approvals=ref<any[]>([]),initiatedApprovals=ref<any[]>([]),detail=ref<any>(null),capabilities=ref<any>({tools:[],skills:[]})
...
async function openNotices(){showProfile.value=false;showNotices.value=true;await nextTick();updateNotificationPosition();noticeLoading.value=true;try{[notices.value,approvals.value,initiatedApprovals.value]=await Promise.all([api('/notifications'),api('/approvals'),api('/approvals/initiated')])}catch(e:any){fail(e.message)}finally{noticeLoading.value=false}}
