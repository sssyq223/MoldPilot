...
import { MessageSquare, Plus, Search, Bell, Paperclip, PanelRight, Maximize2, Minimize2, ArrowUp, Folder, ShoppingCart, Settings, Bot, ChevronRight, LogOut, X, Square, ArrowRight, Pin, Archive, Copy, Check } from 'lucide-vue-next'
...
const settingsOpen=ref(false),showProfile=ref(false),noticeLoading=ref(false),showSidebarSearch=ref(false)
...
function escapeMenu(e:KeyboardEvent){if(e.key==='Escape'){if(showProfile.value)closeProfile();contextPopoverOpen.value=false}}
...
async function restore(){const response=await api('/me');me.value=response.user;permissions.value=response.permissions;modelName.value=response.model??'未配置模型';modelLimits.value=response.model_limits||modelLimits.value;await refresh();try{const layout=JSON.parse(localStorage.getItem('mold.layout.'+me.value.id)??'{}');width.value=Math.max(560,Math.min(layout.width??650,window.innerWidth-480));expanded.value=false;panel.value=''}catch{}}
...
<SettingsPage v-else-if="settingsOpen" :me="me" :permissions="permissions" :capabilities="capabilities" :model-name="modelName" :color-theme="colorTheme" @theme-change="changeTheme" @model-updated="modelName=$event" @open-conversation="openArchivedConversation" @close="settingsOpen=false;restore().catch(e=>fail(e.message))" @error="fail"/>
<main v-else class="workbench" :class="{'panel-full':full&&workspaceOpen,'sidebar-collapsed':sidebarCollapsed}">
...
<div v-if="showProfile" class="profile-menu" role="menu" aria-label="账号选项"><p><strong>{{me.display_name}}</strong><small class="muted">{{me.username}}</small></p><button role="menuitem" @click="openSettings"><Settings :size="17"/>设置</button><button role="menuitem" @click="logout"><LogOut :size="17"/>退出登录</button></div></div></aside>
      ...
      <div class="agent-answer message-block">
        <div class="answer-body">
          ...
          <div v-if="runTrace(run).length" class="react-trace" aria-label="执行链路">
                ...
                <template v-for="(item,index) in runProcessTrace(run)" :key="item.id||item.call_id||index">
                  ...
                  <details v-else-if="item.type==='tool'" class="agent-tool-row">
                    ...
                    <div class="agent-tool-body">
                      <p class="agent-tool-meta">{{item.proposal?'等待你核对后打开工作区处理':(shanghai(item.as_of)+' · 按当前权限返回')}}</p>
                      <div v-if="item.proposal" class="agent-tool-main">
                        <ContactProposal :step-id="item.id" :proposal="item.proposal" @open="id=>{contactTarget=id;openPanel('contacts')}"/>
                      </div>
                      <div v-else class="agent-tool-main agent-records">
            ...
            <template v-else-if="!runProcessTrace(run).length" v-for="(item,index) in runTrace(run)" :key="item.id||item.call_id||index">
  ...
  <section v-if="workspaceOpen" :key="me.id+me.authorization_hash" class="workspace" :style="full?{}:{width:width+'px'}"><header class="workspace-header"><strong>工作区</strong><div><button class="icon-button" title="关闭工作区" aria-label="关闭工作区" @click="collapse"><X :size="15"/></button><button class="icon-button" :aria-label="full?'返回对话':'全屏工作区'" @click="full=!full"><Minimize2 v-if="full" :size="15"/><Maximize2 v-else :size="15"/></button></div></header><nav class="workspace-tabs"><button class="active">{{labels[panel]}}</button><button v-if="detail&&panel==='approvals'" class="object-tab">{{numberText(detail.snapshot.number)}}</button><span class="muted small">本地开发 · 模拟资料</span></nav><div class="workspace-content">
  ...
  <div v-if="selectedEvidence" class="modal-shade evidence-modal-shade" @click.self="selectedEvidence=null">
    <section class="modal evidence-modal" role="dialog" aria-modal="true" aria-label="工具完整依据">
      ...
      <div class="evidence-modal-content">
        ...
        <ContactProposal v-if="selectedEvidence.proposal" :step-id="selectedEvidence.id" :proposal="selectedEvidence.proposal" @open="id=>{selectedEvidence=null;contactTarget=id;openPanel('contacts')}"/>
      </div>
    </section>
  </div>
