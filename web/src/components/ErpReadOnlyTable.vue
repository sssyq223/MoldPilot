...
<template>
  <section class="erp-readonly-table" :class="{'is-processing-diff':tableLabel==='ERP 处理后的设计明细'||tableLabel==='ERP 参数处理前后对比'}" :aria-label="tableLabel">
      ...
      <table>
        ...
        <tbody>
          <tr v-for="(row,index) in rows" :key="rowKey(row,Number(index))">
            ...
            <td v-for="column in columns" :key="column.key" :class="{'is-calculation':column.key==='calculation'}" :title="cell(row,column)">
...
.erp-readonly-table.is-processing-diff .erp-readonly-table-scroll{scrollbar-width:thin;scrollbar-color:#8291a2 #252b32;overscroll-behavior:contain}
.erp-readonly-table.is-processing-diff .erp-readonly-table-scroll::-webkit-scrollbar{width:12px;height:12px}
.erp-readonly-table.is-processing-diff .erp-readonly-table-scroll::-webkit-scrollbar-track{background:#252b32}
.erp-readonly-table.is-processing-diff .erp-readonly-table-scroll::-webkit-scrollbar-thumb{border:3px solid #252b32;border-radius:8px;background:#8291a2}
.erp-readonly-table.is-processing-diff .erp-readonly-table-scroll td{max-width:420px;height:auto;min-height:42px;overflow:visible;text-overflow:clip;white-space:nowrap;vertical-align:middle}
.erp-readonly-table.is-processing-diff .erp-readonly-table-scroll td.is-calculation{width:420px;max-width:420px;white-space:normal;overflow-wrap:anywhere;vertical-align:top;line-height:1.55}
