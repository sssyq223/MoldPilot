...
const summaryTitle=computed(()=>receipt.value
  ? `${actionTitle.value}${approval.value?'已提交审批':'已确认'}`
  ...
  <section v-else-if="placement==='message'" class="contact-proposal" aria-label="业务操作建议">
    <div class="proposal-brief">
      <span>
        ...
        <small>{{receipt?(approval?'已提交后续审批，请以最终生效回执为准。':'操作已按回执留痕。'):'点击详情核对字段后再确认，不会自动执行。'}}</small>
      </span>
      <button v-if="proposal.kind==='contact'&&receipt&&receipt.case_id" @click="emit('open',receipt.case_id)">查看材料</button>
      <button v-else-if="!receipt" :disabled="busy" @click="review">{{busy?'正在准备…':'查看详情'}}</button>
    </div>
    <p v-if="error" class="proposal-error" role="alert">{{error}}</p>
  </section>
  <Teleport to="body"><div v-if="intent" class="modal-shade" @click.self="intent=null"><section class="modal proposal-modal" role="dialog" aria-modal="true" aria-label="确认业务操作">
    <h2>请核对本次业务操作</h2>
    <dl><template v-for="(value,key) in intent.display" :key="String(key)"><dt>{{key}}</dt><dd>{{displayValue(String(key),value)}}</dd></template></dl>
