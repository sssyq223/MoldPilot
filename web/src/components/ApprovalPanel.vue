...
import { ArrowRightLeft, Check, Circle, FileText, ShieldCheck, Undo2, UserPlus } from 'lucide-vue-next'
...
<template>
  ...
  <div class="approval-layout">
    ...
    <section class="timeline surface"><h3>审批流程</h3><small class="muted">{{detail.definition.name}} · 第 {{detail.definition.version}} 版</small>
      ...
      <div v-for="(node,i) in detail.nodes" :key="node.key" class="timeline-node" :class="{current:detail.status==='RUNNING'&&i===detail.stage_index,complete:i<detail.stage_index}"><Check v-if="i<detail.stage_index" :size="18"/><Circle v-else :size="18"/><div>{{node.name}}<small>{{nodeState(Number(i))}} · {{node.mode==='ALL'?'会签':'或签'}}</small></div></div>
  ...
  <div class="decision-box" v-if="detail.allowed_actions.length"><h3><ShieldCheck :size="17"/> 本次审批意见</h3><label v-if="detail.allowed_actions.includes('RETURN')">退回目标<select v-model="returnTarget" aria-label="退回目标"><option value="" disabled>请选择模板允许的责任节点</option><option v-for="target in detail.return_options" :key="target.key" :value="target.key">{{target.name}}</option></select></label><textarea v-model="comment" placeholder="填写判断依据和审批意见…" rows="3" aria-label="审批意见"/><div class="actions"><button v-for="action in detail.allowed_actions" :key="action" :class="action==='APPROVE'?'primary':''" :disabled="busy" @click="prepare(action)">{{labels[action]}}</button></div></div>
