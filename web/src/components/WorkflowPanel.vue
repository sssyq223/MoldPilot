    ...
    <WorkflowCanvas :nodes="nodes" :users="users" :groups="assignmentGroups" :selected-index="selectedNodeIndex" :storage-key="key" :simulation="simulation" @select="selectCanvasNode" @add="addNode" @add-person="addPersonNode" @connect="connectNodes" @assign="assignPerson" @unassign="unassignPerson" @error="emit('error',$event)">
      ...
      <template #canvas-overlay>
        ...
        <aside v-else-if="canvasPanel==='add-sign'&&selectedNode" class="workflow-canvas-overlay" aria-label="节点加签设置">
          ...
          <div class="workflow-node-options">
            <label class="workflow-overlay-switch"><input type="checkbox" :checked="!!selectedNode.assignment" @change="toggleDynamicAssignment(selectedNode,($event.target as HTMLInputElement).checked)"/><span><b>动态人员规则</b><small>按组织与业务角色选人</small></span></label>
            <label class="workflow-overlay-switch"><input type="checkbox" :checked="!!selectedNode.add_sign_policy" @change="toggleAddSign(selectedNode,($event.target as HTMLInputElement).checked)"/><span><b>允许加签</b><small>办理时增加复核人</small></span></label>
          </div>
