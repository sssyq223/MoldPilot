...
function addBusinessPermission(n:any,event:Event){const select=event.target as HTMLSelectElement,key=select.value;if(key){toggleAssignmentValue(n,'business_permissions',key,true);if(!n.assignment.responsibility_scope.length&&assignmentDomain.value.responsibility_dimensions?.length===1)toggleAssignmentValue(n,'responsibility_scope',assignmentDomain.value.responsibility_dimensions[0].key,true)}select.value=''}
async function previewAssignment(){if(!selectedNode.value?.assignment)return;const assignment=selectedNode.value.assignment;const needsScope=assignment.domain_roles?.length||assignment.responsibility_scope?.length;if(needsScope&&!previewScopeId.value)return emit('error',`请选择要预览的${assignmentDomain.value.scope_label||'业务范围'}`);const context=needsScope&&assignmentDomain.value.context_key?{[assignmentDomain.value.context_key]:previewScopeId.value}:{};previewing.value=true;try{assignmentPreview.value=await post('/workflows/assignment-preview',{assignment,context})}catch(e:any){emit('error',e.message);assignmentPreview.value=null}finally{previewing.value=false}}
