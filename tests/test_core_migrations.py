...
def test_template_pack_migrates_real_postgres_without_mold_tables():
    ...
    try:
        ...
        assert "a10c0e000008 (head)" in migrate("current")
        ...
        with probe.connect() as connection:
            ...
            action_columns = set(connection.execute(text(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_schema=:schema AND table_name='approval_action'"
            ), {"schema": schema}).scalars())
            assert "decision_context" in action_columns
            candidate_columns = set(connection.execute(text(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_schema=:schema AND table_name='approval_candidate'"
            ), {"schema": schema}).scalars())
            assert {"instance_id", "stage_index", "user_id", "status", "version",
                    "seat_id", "claimed_at"} <= candidate_columns
            timer_columns = set(connection.execute(text(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_schema=:schema AND table_name='wf_timer'"
            ), {"schema": schema}).scalars())
            ...
            escalation_columns = set(connection.execute(text(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_schema=:schema AND table_name='workflow_escalation_task'"
            ), {"schema": schema}).scalars())
            assert {"timer_id", "instance_id", "stage_index", "node_key", "user_id",
                    "status", "version", "closed_at", "close_reason"} <= escalation_columns
