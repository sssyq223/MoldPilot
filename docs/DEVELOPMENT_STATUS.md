...
## 持续开发：资料模板 XLSX 解析预览（2026-09-15）
...
- 该接口只做预览，不创建业务材料绑定、不替代人工确认、不解除正式提交的 `MATERIALS_NOT_BOUND` 门禁；后续仍需补持久化资料绑定、字段级权限、版本冻结、业务对象关联和前端核对界面。
- 新增测试覆盖成功解析临时映射、保存映射版本后复用预览、模板版本哈希冲突阻断，以及公式单元格、重复行号等必须待核对的情况。
...
- BPM 节点新增显式布尔标记 `agent_auto_approval` 和可选安全条件 `agent_auto_policy.condition`。只有流程节点允许自动审批、节点安全条件明确满足、当前席位用户存在有效委托、当前仍有审批权限、材料完整且规则允许 `APPROVE` 时，运行时才会调用同一套审批决定逻辑自动同意。未配置、标记为 false、安全条件不满足或资料不足的节点继续强制人工办理。
...
- 前端已在输入框工具栏接入 Agent 权限模式下拉，选择不会跳转设置页，会按当前用户本机持久化并随 `/api/runs` 写入本轮 Run checkpoint；worker claim、运行历史和中间 checkpoint 均保留 `agent_permission_mode`，用于区分“每次询问”和“按授权自动审批”的运行语境。
- 设置页提供个人自动审批授权/撤销，并在审批流程配置节点中提供 `agent_auto_approval` 开关和安全条件编辑；授权选项只来自已发布且明确允许自动审批的节点。
- 新增 SQLite 单元测试覆盖显式允许节点自动同意、强制人工节点不被委托绕过、节点安全条件阻断自动审批、委托撤销改变授权指纹、API 只暴露/接受显式自动审批节点，以及输入框 Agent 权限模式可进入 Run 历史与 worker 上下文。管理员策略模板、批量委托、通知摘要和正式验收仍待补齐。
...
- 新增 SQLite 单元测试覆盖有效整套委外路径、委外合同、供应商执行/收货检验、质量延期扣款联络、供应商付款、客户验收清单、多候选不自动决定和无订单工具不泄露发货单。FR-070～077 仍为 NOT_VERIFIED：真实供应商节点模板/频率、合同签署文件、客户资料交接、设变议价、责任确认、复验关闭、扣款结算和 ERP 财务联调尚未完成验收。
...
- 新增 SQLite 单元测试覆盖供应商发货、仓库收货、入库检验、出库移动、试模通过、客户验收清单、客户验收质量联络、多候选不自动决定和无订单权限不泄露物流单号。FR-064～069 仍为 NOT_VERIFIED：真实出厂检验、固定物流路线/承运商/车型/价格有效期、本次结算价格、客户签收、客户验收复验、费用扣款和 ERP 出入库/发货联调尚未完成验收。
