...
- 验证：仓库隔离临时目录下完整后端回归 `495 passed`，Python 编译、mold/template 两套 Vue 类型检查与生产构建、正式 PostgreSQL `upgrade/current/check` 均通过，mold 迁移 head 为 `f40e6c8a3d46`、template/core head 为 `a10c0e000007` 且无模型漂移。core 迁移链在隔离 PostgreSQL schema 完成 `base → head → check → base`。内置浏览器实际完成“新建工作日历 → 配置工作周、午休分段、休息日与补班日 → 发布第 1 版 → 新建流程 → 指定审批人和 24/2 小时时限 → 绑定日历版本 → 保存并校验发布”，干净验收标签页 console warning/error 为零。计时器触发与工作小时跨日计算由隔离 PostgreSQL 集成测试验证，本次未等待真实 24 小时，也不代表生产 Redis/部署验收。
- 尚未完成：升级与抄送策略、除人员分配和计时器失败以外的通用故障恢复、独立流程服务部署、Redis 生产运行观测。该批不能表述为 V3.6 §12.7 或完整 BPM 已全部交付。
...
当前 BPM 仍有明确缺口：项目角色和基于实际资料的候选人预览、完整业务字段目录、升级/抄送策略、除人员分配与计时器失败外的通用故障恢复、独立流程服务部署以及完整图形设计器。模板限定的候选人唯一领取、申请人/前序责任节点退回、受控席位转交、显式人员池内的前/后加签、受限人工代理、节点持久化提醒/到期标记、版本化工作日历和人员分配阻塞恢复已经完成；当前已发布模板冻结版本、动态角色/部门解析、指定用户 ALL/ANY/CLAIM、受限字段的前向条件分支和模拟可验证，仍不代表通用 BPM 全部交付。
...
| BPM | SpiffWorkflow 顺序节点、ALL/ANY/CLAIM、指定审批人、版本冻结、强制驳回、模板限定的候选领取/指定退回、无审批人阻塞与人工重试、受控转交、前/后加签、受限人工代理、节点持久化提醒与到期标记、版本化工作日历 | 完整设计器、升级/抄送、其他故障恢复、独立部署及全部节点可用性验证 |
