...
- 新增 SQLite 单元测试覆盖默认“每次询问”不触发自动审批、显式 `delegated_auto` 才允许节点自动同意、强制人工节点不被委托绕过、节点安全条件阻断自动审批、委托撤销改变授权指纹、API 只暴露/接受显式自动审批节点，以及输入框 Agent 权限模式可进入 Run 历史与 worker 上下文。管理员策略模板、批量委托、通知摘要和正式验收仍待补齐。
