...
- 预览、映射和核对包仍不创建业务材料绑定、不作为审批实例材料冻结、不解除正式提交的 `MATERIALS_NOT_BOUND` 门禁；后续仍需补业务对象关联、字段级权限、审批发起时的材料冻结和前端核对界面。
- 新增测试覆盖成功解析临时映射、保存映射版本后复用预览、模板版本哈希冲突阻断、公式单元格/重复行号进入待核对，以及干净核对包确认成功、有 issues 的核对包禁止确认。
