...
- 已新增本地 `engineering_change_intake` 与 `engineering_contact_collaboration` Skill；前者提供本地设变承接查询及三个 Proposal Tool，保存客户/内部/委外分类、执行方式、收费与合同状态、原模具分支、客户模号历史和本地关联版本。首次外部模具必须进入人工承接分支，不能伪造原模具或 ERP 外部 ID。
- 已新增 `model_provider_configuration` 与 `document_model_configuration` Skill；模型目录查询、供应商目录检测和模型保存/默认切换均通过本地 Tool 注册，管理员权限、revision 和文档模型占用门禁沿用本地配置服务，API Key 不进入 Tool 参数或返回值。
- 开工/合同后置绑定的本地范围已阻断 ERP 核算清单引用，改为只接受本地核算资料对象；未实现 ERP 清理，远程 ERP 源文件未修改。
- 本地设变 ORM/Tool 已注册进当前活动 ORM 元数据，领域迁移源已补齐至 `mb0d0e000017`；本机库仍为 `mb0d0e000016` 且未获迁移批准，启动预检在缺少 `local_change_*` 表时继续隐藏该组 Tool，不影响既有服务启动。未执行数据库迁移、未修改正式库、未运行前端构建、未联调 ERP。
- 当前自动化证据：本地范围/能力/无 ERP/模型配置/设变承接/工程联络/Proposal 确认链定向测试 `26 passed`；模型目录、发现和运行选择回归 `57 passed`；启动预检只读核对通过。这些是代码级证据，不等于正式库、真实供应商、页面或完整业务验收。

## 中标到开工通知工作流（2026-09-23，当前状态）

- 已实现：确认 `BID_NOTICE` 后由 `bid_to_start_notice` Skill 通过受信任事件 Tool 自动创建仅供超级管理员处理的内部开工通知草稿；项目部（当前由超级管理员兼任）先核对中标资料并人工确认内部承接、整套委外或拒绝，不在承接决定前要求项目；承接后再从系统候选选择项目，填写内部模具编号作为后续合同/核算清单匹配线索，并通过 `prepare_admin_start_department_dispatch` 分发给设计/采购/制造/装配/财务五类部门，逐部门通过 `prepare_admin_start_department_ack` 记录“已收到/未收到”回执，回执状态仅展示、不阻塞决定（B 方案）；分发、回执、修改和决定全部追加版本留痕；承接/委外后合同 OCR 自动生成匹配候选，正式关联仍需人工确认，不能把人工编号直接当作当前 `ProjectMold` ID。既有正式 `StartNotice`、部门回执和 `PostStartBinding` 门禁保留为后续阶段。
- 已补充：`query_admin_start_notices`、`prepare_admin_start_notice_update`、`prepare_admin_start_notice_decision`、`prepare_contract_match_confirmation` Tools 及 `admin_start_notice.execute` Proposal Handler；普通项目部门初始不生成回执；旧 `BusinessSubject(kind='internal_start')` 仍仅作为 `compatibility_only` 只读引用。
- 迁移边界：`mb0d0e000014_bid_start_workflow.py`、`mb0d0e000015_admin_start_workflow.py` 和 `mb0d0e000016_admin_start_departments.py` 是当前活动迁移源；正式库迁移本轮未执行，隔离测试库只按测试夹具验证。本机 agent_db 已用一次性 SQL 补齐 mb016 结构并戳记 mb0d0e000016（SQL 已删除）。
- 核算清单：已接入 ERP `production_cost_sheet_file/{id}/binding-reference` 只读引用，使用 ERP `file_hash` 作为外部版本指纹；Agent 不复制清单内容、不改变 ERP 状态，非稳定 ERP 档案引用仍安全阻断。真实 ERP 服务联调尚未进行。
- 验证：本轮事件触发、草稿/分发/回执/决定、合同候选、Tool 契约、既有中标和迁移拆分定向回归 `28 passed`；Tool Gateway/Harness/能力目录回归 `142 passed`；本机运行库预检、compileall 和本批路径差异检查通过。正式迁移、真实部门负责人权限映射、前端页面验收和完整业务验收仍未完成。


## 中标到开工通知批次 A：分类证据基础（2026-09-21）

- 新增页级全文确定性分类器：同时识别文档类型、业务事件、证据、冲突和三态结果；中标自动确认要求中标语义、项目上下文、模型一致且来源元数据齐全，合同/开工通知冲突进入人工复核。
- 文档模型分类改为提交全文文字块，严格校验证据页码、摘录、冲突、抽取字段和分类器版本；旧的 `Classification(type, confidence)` 调用保持兼容。
- 预分类完成事件保存分类证据快照，原有人工类型确认仍是唯一业务确认入口；没有新增表、没有执行迁移、没有触发承接/开工/合同写入。
- 登记 `contracts/document-classification.schema.json`、`contracts/bid-to-start-notice.skill.json` 和 `bid_to_start_notice` Skill 规格。新增合成分类回归 5 项，现有合同 OCR 回归 23 项通过。
- 已增加按文件版本读取分类快照的只读接口；尚未实现分类记录独立追加表、分类人工确认 HTTP 命令、中标确认事件到草稿 Skill 的正式发布链路，以及真实来源元数据接入。这些属于后续批次，不能据此声称已生成正式开工通知。

更新：2026-09-20。此表记录已实现与未实现的差异，不缩减 V3.6 全量范围，不把业务功能分产品阶段，也不替代正式验收。

## 文档模型截断与状态展示修复（2026-09-21）

- 根因：文档提取固定每 4 页请求；供应商流式响应 `finish_reason=length` 后，旧逻辑仍原批次重试，容易重复消耗同一输出预算。`DocumentActivity.vue` 又直接展示 job 的 `last_error`，因此重试处理中仍显示上一次“模型输出被截断”。
- `ocr_provider.py` 现对输出截断或输入预算超限的批次递归二分，最小到单页；单页仍超限返回 `DOCUMENT_MODEL_BATCH_TOO_LARGE`，本轮调用达到上限返回 `DOCUMENT_MODEL_CALL_LIMIT`。输入护栏 32768 字节、总调用上限有界，未提高 8192 输出预算、未放宽来源校验、未丢弃无效字段。
- `document_batch_cache.py` 保存拆分凭据、复用已完成子批次，并记录白名单调用统计（页码、耗时、finish_reason、token usage、重试数等），不保存原文、提示词、Key 或模型原始响应。`DocumentActivity.vue` 区分当前第 N 次处理中、上一次失败和最终失败。
- 新增 `tests/test_document_adaptive_batches.py`：截断拆分、输入预算、单页终止、调用统计脱敏、来源校验、调用上限、重试复用和终态展示回归共 **8 passed**。合同 OCR/自动入口相关回归 **39 passed**；思考参数回归 **13 passed**。均使用隔离 `127.0.0.1:55432/moldpilot_test`，只读替换迁移检查，未迁移/清库生产库。
- 已重载 API 与 Document Worker；API/前端代理/PaddleOCR 健康仍 200，日志无 ERROR/Traceback。当前原合同任务仍保持此前自然完成状态，未代重试、未重复上传。未运行前端 build/typecheck/浏览器，页面视觉仍待潘总刷新验收。

## 启动恢复：独立 PostgreSQL 服务与只读预检（2026-09-21）

- 已确认启动失败原因：55432 项目 PostgreSQL 进程未运行，日志末次异常终止在 09-20 19:58；Windows 的 5432 服务属于另一数据目录。一键启动原来在启动 API 前无条件执行迁移，数据库不就绪时无法进入 API 启动。不是候选字段弹窗引起的后端导入失败。
- 经潘总批准，新增 `scripts/check_runtime.py`，将 `一键启动.bat` 的自动迁移替换成只读连接、版本及必需表列核验。连接超时 5 秒、SQL 超时 5 秒、外层子进程总时限 20 秒；Windows 沿用 CPython venv 子进程启动方式，避免超时只杀掉重定向父进程。目标固定解析到本机回环，不允许 URL 查询参数把探测转向远端；不回显连接凭据或子进程异常正文。`--check` 不启动应用、失败不等待按键。
- 完整冷备份 `D:/MoldPilotBackups/startup_repair_20260921_083130/pgdata-offline.zip` 已做 ZIP CRC/SHA256 核验；D 盘备份目录 ACL 限当前用户、SYSTEM、Administrators。配置备份与恢复清单 `.local/backups/startup_repair_20260921_083130/manifest.json` 含敏感备份引用，不输出备份内容。
- 新增 `scripts/install_local_postgres_service.ps1`。经 UAC 批准注册 `moldpilot-postgresql-55432`，原数据目录 `.local/pgdata`、端口 55432、NetworkService 账号、自动启动及 5/15/60 秒失败重启；数据库正常恢复后已接受连接。原 5432 服务未变。当前 E 盘为 exFAT，Windows 卷健康报告 Warning；这不能单独归因为本次进程终止，未执行磁盘修复或数据迁移。后续宜单独规划 NTFS 数据目录和盘健康复核。
- 必要 pytest `tests/test_runtime_preflight.py` **11 passed**，其中实际 PostgreSQL 校验只读取 55432/moldpilot_test，无迁移、建表或清库。修改后的真实 `一键启动.bat --check` 返回 0，用时约 7.38 秒；正常应用入口由独立进程恢复，API/5173 代理/PaddleOCR/网页入口均 HTTP 200，四后端服务及 Vite 开发服务持续存活，PG 会话仍为本机 agent_db。core a10c0e000008 / mold mb0d0e000013，.env/模型配置哈希未变；无新增业务确认、重试或模型调用。
- 未重启整台电脑、未人为杀死数据库验证故障恢复，故仅确认自动启动/恢复策略已配置，不声称真实开机验收通过。未运行前端构建、浏览器或页面业务验收。

## 持续开发：供应商多模型目录与会话思考档位（2026-09-20）

- 新增供应商连接/模型条目的 v3 配置层。旧 v2/legacy 只读投影，原 profile ID 保留；首次本人保存才转换，Key 在同一供应商下只保存一份。停用且未填完的旧草稿不会拖累有效模型；revision 冲突、线程/进程文件锁、原子替换和文档引用保护已接入，禁止旧接口覆盖 v3。未自动改写真实配置。
- 设置页改为独立 `ModelProviderSettings.vue`：检测目录、搜索、多选原子添加、手动添加、模型参数与明确的系统默认设置。检测仅 GET `/models` 或本机 `/api/tags`，不生成、不自动保存、不将目录视为模型调用/工具能力证明。DNS 核验后固定目标 IP，保留 TLS 主机名、代理 CONNECT/SNI；禁止重定向、限制 2 MiB/1000 条/并发，拒绝压缩响应以避免无界解压。换地址不默带旧 Key，错误不回显凭据和上游正文。
- 聊天 `ModelSelector.vue` 按供应商分组、离散思考滑条、默认恢复、用户/会话隔离偏好，未知模型不开放猜测档位。GLM 低/高/最高对应 low/high/max；管理员可显式声明原生 OpenAI 推理、Ollama 开关/档位模板，但模板不等于供应商实测。原生 OpenAI 用 reasoning_effort/max_completion_tokens 且不附带固定 temperature；Ollama 用 think。没有自动提高既有输出额度。
- Run 创建时保存非敏感宿主选择和审计，领取/恢复时校验原模型及路由。Worker 领取后按任务解析凭据与参数，不先用全局模型决定所有任务。checkpoint 不能覆盖模型选择；全局默认变化、凭据轮换、审批续办及排队模型删除/停用均有回归。普通用户权限未扩大，不能利用错误差异枚举其他模型。聊天选择不改变文档固定 GLM profile/low。
- 组合 12 文件 `263 passed`；最后后端修改后 9 文件 `216 passed`（241.64s，7 个依赖弃用警告）。使用隔离 PostgreSQL 串行 pytest，迁移钩子仅检查现有表；配置写入全部使用临时文件，HTTP/DNS 为替身，代理测试执行真实 HTTPX/httpcore CONNECT 链但替换网络流。未运行迁移、前端构建、浏览器或真实供应商探测，不能称实际接口和页面已验收。
- 本机加载与备份清单 `.local/backups/model_catalog_20260920_190506/deployment.json`，备份包含 Key，禁止输出内容。真实配置保持 v2，.env 及 Key 未变，原失败文档未代重试。真实目录、不同模型生成兼容性、前端视觉/交互、多进程配置写压力及独立计费测试调用按钮仍未验收/未提供；详见对应规格与计划。

## 持续开发：上传自动预分类与原会话确认（2026-09-20）

- 本节替代历史“上传必须创建 Agent Run、确认接收 proposal 后才开始预分类”的入口要求。单文件和原子 multipart 批量上传在文件事务内直接创建预分类作业，支持请求幂等和失败回滚；不创建 Run/Step/HumanIntent。既有 8 个 Tool 和唯一 Skill 保留为辅助办理与存量兼容，不是解析入口。
- 分类后在原会话提供类型/合同分组核对与两阶段 HumanIntent。本人确认才排全文任务，确认时重验归属、授权指纹、安全版本、内容摘要、归档状态和凭证；重复确认不重复排队。任务卡展示阶段、耗时、缓存页数、重试和错误，候选与来源不等于已确认业务事实。前端代码已接入，页面效果尚待用户验收。
- 文档 Provider 使用流式纯文本模型，保留本地 PaddleOCR/文本层分析边界。结构契约由正式字段目录生成；页码、原文、bbox 和 HEADER 行标识从可信来源推导。来源错误整批有界重提取一次，仍失败则拒绝，不静默删除错误字段。已校验批次暂存在候选表，完成凭据仅记指纹/页码/引用；成功前不提供可复核字段，重试复用匹配输入/模型/协议的批次，包含空批次及凭据清理后恢复。
- 文档模型固定引用 GLM-5.3-Flash profile、reasoning_effort=low，不随聊天选择切换；每个新作业重新解析该 profile，支持凭据更新。生成过程续租，失租不覆盖新领取者。空闲读取超时与流式进度中的总时限检查分离；未证明严格墙钟中断正在阻塞的读取，不将其描述为硬截止保证。
- 完成/失败事件指定创建人为收件人，投递和读取时重验文件权限；通知带受权限约束的原会话定位。原会话首次聊天也获得当前可读文件引用，不伪造历史 Run 或本轮附件。
- 验证：14 个测试文件组合 `262 passed`；后续配置刷新修改后相关回归 `176 passed`。最后补充凭证过期/等长篡改、流式总时限/空闲错误码和失租检查：两文件运行 `28 passed / 1 failed`，失败是测试凭证过短被格式校验先拒绝；改成等长篡改后单项 `1 passed`。上述均为本机隔离 PostgreSQL 串行 pytest，迁移钩子只读检查现有表，没有执行迁移。
...
## 持续开发：销售合同 PDF 接收、OCR 复核与两级审批（2026-09-19）

- 复用聊天回形针上传 PDF；上传批次创建带可信 `ATTACHMENT_UPLOAD` 标记的 Agent Run，Harness 按服务端 PDF MIME 激活唯一 `sales_contract_intake` Skill，并把完整 `SKILL.md` 注入模型上下文，不按文件名或固定提示词猜业务。
- 同一 Skill 编排 8 个受控 Tool，覆盖本 Run 附件查询、接收 proposal、状态查询、类型/合同分组确认、失败 OCR 重试、合同识别查询、字段/项目/模具/关系复核及合同登记。全部写操作必须经本人 proposal 确认；OCR Worker 独立异步执行，Run 不轮询。
- 文档管线已改为 `PyMuPDF → 按需本地 PaddleOCR GPU → 不可变页面文字缓存 → Qwen3 纯文本结构化`：文本型 PDF 直接使用带编号文本块，图片/混合页调用仅监听 `127.0.0.1:18081` 的 PP-OCRv5 mobile 服务；Qwen3 请求不包含 PDF、PNG、图片 URL 或 base64。字段保存机器原值、标准化值、来源块、置信度、来源文件和页码。人工必须确认全部字段、已有项目及每条正式内部模具映射；项目客户关系缺失、OCR 客户冲突、付款节点超过合同额或模具未映射时阻断。
- 本人确认后创建销售合同、统一不可变 `ContractAttachment` 原件关系、模具明细和付款节点，并严格提交“业务主管 → 财务负责人”两级项目角色流程；全部通过前合同不生效。重复合同只关联既有合同，修订/替代关闭旧有效合同，补充合同与原合同并行计入当前金额，历史回款不改写。
- 合同 PDF 一旦关联合同即按项目 `sales_contract.read` 权限读取；撤权后上传者不能凭上传身份绕过。文档接收、分类确认、OCR 成败、人工复核、原件关联和版本关系均记录脱敏审计。
- 旧 `contract_intake_api.py`、合同专用上传/复核页面、菜单、跳转和前端辅助模块已删除，用户侧只保留通用会话、附件、通知与 ProposalCard。迁移已进入 mold domain 链 `mb0d0e000011`，不再挂旧冻结迁移链。
- 隔离 PostgreSQL 已覆盖双 PDF、三类页面分析、页面缓存复用、Qwen 失败重试不重复 OCR、项目/模具复核、proposal、两级审批、生效和撤权读取。本机 RTX 3050 已验证 PaddleOCR 3.7.0 / PaddlePaddle 3.2.0 / CUDA 12.6，3 份匿名真实图片 PDF 已完成 GPU OCR → Qwen3 纯文本分类；其中大画幅页面按比例限制在服务安全像素内。现有样本均分类为 `OTHER`，因此真实销售合同字段提取、文本/混合/旋转/多页真实样本、正式数据库变更、正式主数据/角色/流程及用户实际操作仍为 `NOT_VERIFIED`。
