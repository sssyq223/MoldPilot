...
- 本轮迁移分包验证：新增 PostgreSQL 空 schema 集成门禁实际执行 template 的 `upgrade head → current → check → downgrade base`，确认 `a10c0e000001` 基线可逆且不创建任何模具表；默认 mold 分发仍定位 `d5b8f3c20e71 (head)` 并通过 check。迁移 URL 统一保证进程级 `AGENT_MIGRATION_URL`/旧别名优先于 `.env`，测试夹具显式覆盖并恢复新变量，避免配置新变量后测试误向正式库执行迁移；对应优先级回归已纳入门禁。合并 ERP 设计上传撤回及前端调整后，清除了撤回遗漏的 manifest/测试引用，最终完整后端回归 451 项全部通过、前端类型检查与生产构建通过；真实后端重启后内置浏览器成功恢复历史会话、已处理确认卡和最终正文，控制台无 warn/error。本轮仍未增加业务功能或传统 ERP 页面。
