...
- 无数据库结构变更：规则继续存入版本化流程配置 JSON。完整后端回归 `504 passed`，mold/template 两套 Vue 类型检查与生产构建均通过。内置浏览器实际完成“选择采购申请审批能力 → 自动绑定当前项目责任域 → 与采购负责人规则取交集 → 预览唯一有效账号 → 保存并发布第 2 版 → 重新打开验证持久化”；发布状态和版本历史正确，控制台 warning/error 为零。
- 尚未完成：mold 包目前只把项目责任域开放为流程候选维度；品类、仓库等更细责任维度仍由真实材料办理时逐项复验，尚未作为可配置候选维度展示。配置端也还没有独立的批量“资格缺口”报告，这些不能表述为完整责任域治理已经交付。
