...
- 新增 mold 领域包只读协调工具 `query_project_kickoff_context` 和“项目启动链路协调”Skill，把承接确认、销售合同、正式开工、项目基线计划作为四类独立事实形成一个链路投影。投影返回当前阶段、阶段状态、真实依据、阻塞项、主线下一步与可并行事项；不会合并单据、不会自动跨审批、不会创建 ERP 任务。
...
- 对话工具依据新增“项目启动链路”表格，展示四阶段状态、已知事实、阻塞说明和下一步能力；建议区分主线/并行，并明确哪些动作需要本人确认。没有新增菜单或传统 ERP 列表页。
