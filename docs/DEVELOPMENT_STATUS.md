...
- 按 V3.6 §505 补齐“业务能力 + 责任范围”候选规则。通用 BPM 只保存业务包发布的权限键、责任维度键和运行上下文，不认识采购、项目或模具语义；mold 包当前发布全部领域权限与“当前项目”责任维度，template 包保持中性空目录。
...
- 前端能力嵌入既有 BPM 节点快捷设置，不新增菜单或 ERP 式页面。业务能力以可移除标签呈现；单一责任维度会在首次选择能力时自动启用，预览明确提示实际材料仍会再次复验。通用组件没有硬编码 `project_id`、`PROJECT_OWNER` 或“项目角色”。
...
- 无新增数据库结构：规则继续存入版本化流程配置 JSON。完整后端回归 `507 passed`，mold/template 两套 Vue 类型检查与生产构建均通过。内置浏览器刷新后历史会话正常恢复；随后完成“管理部门候选池 + 采购申请审批能力 + 当前项目责任域”预览，显示 2 位有效账号和 1 位明确的缺少授权候选人，退出未保存草稿后没有产生测试数据。
- 尚未完成：mold 包目前只把项目责任域开放为流程候选维度；品类、仓库等更细责任维度仍由真实材料办理时逐项复验，尚未作为可配置候选维度展示。资格缺口已支持单节点即时预览，但尚无跨全部模板的独立批量治理报告，这些不能表述为完整责任域治理已经交付。
