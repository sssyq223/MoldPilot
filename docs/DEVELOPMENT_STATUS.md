...
当前 BPM 仍有明确缺口：项目角色和基于实际资料的候选人预览、完整业务字段目录、超时与事件恢复、独立流程服务部署以及完整图形设计器。模板限定的候选人唯一领取、申请人/前序责任节点退回、受控席位转交、显式人员池内的前/后加签和按节点/决定/有效期约束的一跳人工审批代理已经完成；当前已发布模板冻结版本、动态角色/部门解析、指定用户 ALL/ANY/CLAIM、受限字段的前向条件分支和模拟可验证，仍不代表通用 BPM 全部交付。
...
| BPM | SpiffWorkflow 顺序节点、ALL/ANY/CLAIM、指定审批人、版本冻结、强制驳回、模板限定的候选领取/指定退回、无审批人阻塞、受控转交、前/后加签、受限人工代理 | 完整设计器/条件路由、超时恢复及全部节点可用性验证 |
