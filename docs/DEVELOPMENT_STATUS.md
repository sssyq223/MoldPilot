...
- BPM 节点新增显式布尔标记 `agent_auto_approval`。只有流程节点允许自动审批、当前席位用户存在有效委托、当前仍有审批权限、材料完整且规则允许 `APPROVE` 时，运行时才会调用同一套审批决定逻辑自动同意。未配置或标记为 false 的节点继续强制人工办理。
...
- 前端已在输入框工具栏接入 Agent 权限模式下拉，在设置页提供个人自动审批授权/撤销，并在审批流程配置节点中提供 `agent_auto_approval` 开关；授权选项只来自已发布且明确允许自动审批的节点。
- 新增 SQLite 单元测试覆盖显式允许节点自动同意、强制人工节点不被委托绕过、委托撤销改变授权指纹，以及 API 只暴露/接受显式自动审批节点。管理员策略模板、风险阈值、批量委托、通知摘要和正式验收仍待补齐。
