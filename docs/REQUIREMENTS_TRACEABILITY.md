...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：query_quote_acceptance_context 将报价承接决定、后续承接结果和开放中的报价决定按项目上下文返回，供会话查询核对；当前只查询已登记业务单据事实，不生成报价版本、不提交客户反馈、不覆盖原报价依据
- 验证证据：tests/test_quote_tools.py 覆盖有效承接、开放决定集合和项目线索定位；报价提交版本与客户反馈全流程尚未验收
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：query_business_object_candidates 提供中标/承接前的候选项目匹配；query_quote_acceptance_context 可复用授权候选匹配定位项目并返回承接上下文；多候选返回候选列表并要求人工明确项目 ID，不虚填历史报价或历史模具
- 验证证据：tests/test_business_matching.py 覆盖候选匹配与权限边界；tests/test_quote_tools.py 覆盖上下文多候选不自动决定
- 验收状态：NOT_VERIFIED
