...
- 实现证据：`project_control_tools.py` 会话查询/暂停建议/人工确认提交 BPM；`ProjectPauseDetail` 冻结计划与未完成任务；暂停生效沿用项目执行门禁
- 验证证据：`tests/test_project_pause.py` 范围变化阻断；非 PostgreSQL 全集 109 passed；Vue 生产构建通过；专用 PostgreSQL 与 ERP 联调待执行
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：恢复强制引用当前暂停区间；按实际天数顺延冻结的未完成任务；`PauseTaskShift` 保存前后日期，唯一约束与状态门禁防重复
- 验证证据：`tests/test_project_pause.py` 覆盖仅顺延未完成任务、已完成任务不变、重复恢复阻断；迁移 DDL 离线生成通过
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：`PauseRecord` 保存暂停区间和客户交期快照；恢复明确不修改 `ProjectProfile.customer_due_date`；暂停期保留资料、沟通、联络与结算类受控流程
- 验证证据：`tests/test_project_pause.py` 验证客户承诺交期保持不变及节点顺延审计；真实业务验收待执行
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：`ContactCase.problem_source/current_stage/change_type/urgency` 及联络创建输入校验，统一承载来源类别和当前环节；旧记录字段保持可空以避免伪造迁移事实。
- 验证证据：`tests/test_contact_impact.py` 覆盖必填主数据和未来申请日期阻断；尚未覆盖全部类别的真实业务验收。
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：`contact_models.py`、`contacts.py` 补齐客户、模具、料品、申请日期、类别/紧急程度以及事项实际时间、工时、金额、证据、来源；既有附件、版本、审批链继续复用。
- 验证证据：`tests/test_contact_impact.py` 覆盖结构化创建、实际反馈和不可覆盖；生产附件与专用 PostgreSQL 尚未验收。
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：附件版本、时间线、独立复验既有闭环继续保留，责任事项增加实际工时/金额和执行证据；`affected_type/ref` 可关联返工任务、节点或成本对象。
- 验证证据：结构化影响与执行单测通过；财务计价、正式成本过账及真实关联对象联调未完成。
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：联络创建向项目负责人发送待办；事项创建向责任部门负责人派发，处理方案走可配置 Agent BPM 并支持退回整改；方案生效通知当前责任人和部门负责人。
- 验证证据：生命周期集成测试覆盖方案审批与生效；项目负责人/设计/审批岗位的完整生产矩阵仍待真实组织验收。
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：`ContactTask` 结构化列明图纸、物料、采购单、在制任务、供应商任务、节点、合同等对象，并记录继续、暂停、取消、返工或重新下达、交期天数、预计金额及 ERP 来源时点。
- 验证证据：`tests/test_contact_impact.py` 覆盖结构冻结及 ERP 来源约束；尚未逐类联调真实对象的可变更性。
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：方案材料冻结主数据与影响项；审批生效追加 `RESOLUTION_EFFECTIVE` 交接记录、版本和通知，不覆盖历史。ERP 正式版本/任务/计划执行明确留给对应工具回执。
- 验证证据：生效交接幂等单测通过；真实 ERP 更新和全部门通知矩阵未完成，因此不宣称已执行。
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：影响对象类型与原生编号可引用设计、采购、制造、装配、试模相关对象，实际反馈可保存检测/复验依据；不在 Agent 重建 ERP 单据。
- 验证证据：引用与来源约束单测通过；各类正式对象执行工具的端到端关联尚未完成。
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：结构允许引用项目节点、合同和其他原生对象并记录交期/金额影响；方案交接明确不直接改写物流、合同或财务数据。
- 验证证据：仅完成结构和边界校验；物流、运费、合同付款条件、财务及采购预警真实联动未验证。
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：方案审批、实际执行反馈、独立复验/退回整改和人工关闭为分离状态；关闭要求全部有效事项在最新方案下复验合格，实际时间/工时/金额/依据只追加。
- 验证证据：联络生命周期及 `tests/test_contact_impact.py` 覆盖批准不等于完成、反馈、复验和关闭门禁；节点/费用/合同实时阻断尚未完整联调。
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：`backend/app/project_closure.py` 区分终止和正常关闭清单并在终止生效后停止 Agent 本地未完成任务；`project_closure_tools.py` 提供终止材料准备、人工确认与 Agent BPM 提交；`ProjectClosureDetail` 保存终止依据、当前环节、完成工作和费用汇总
- 验证证据：`tests/test_project_closure.py` 覆盖终止生效、任务停止和终止清单建立；内置浏览器验证合成终止建议、本人确认、审批通过及生效
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：终止清单分别保存未完成采购、在制品、供应商任务、客户/供应商结算和收付款处置；ERP事实要求原生引用和截至时间，不复制ERP台账；项目终止后使用专用待结算状态
- 验证证据：`tests/test_project_closure.py` 覆盖ERP来源缺少引用时阻断及追加修订历史；真实ERP处置与结算联调待执行
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：终止关闭使用独立 `TERMINATION` 清单和 `SETTLEMENT_CLOSE` 决定；交付/验收项允许在记录原因后标为不适用；关闭结果为 `CLOSED_TERMINATION`，清单和历史不删除
- 验证证据：`tests/test_project_closure.py` 覆盖终止结算关闭路径、不适用项和关闭后历史保护；专用PostgreSQL验收待执行
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：正常关闭清单独立要求计划、交付、适用验收、发票、客户回款、供应商结算、异常处理及归档；最终审批生效前重新检查实时阻断项，不接受单一完成事实
- 验证证据：`tests/test_project_closure.py` 覆盖未完成计划和新出现联络事项阻断正常关闭，以及计划完成后的实时复核与系统修订归档
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：`ProjectClosureItemRevision` 为每次清单更正追加历史；关闭案例保留创建人、关闭人、版本、来源、依据、截至时间及完整检查明细；迁移 `f1a4d8c7e2b3` 建立关闭案例、清单、修订和强制约束
