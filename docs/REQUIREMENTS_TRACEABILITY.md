...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：query_bid_intake_context 按项目线索核对客户分类、客户规则键、可见合同、模具关联、承接/拒单、开工和已确认中标接收版本；prepare_bid_intake_draft 只接受当前会话明确绑定的文件，按中标通知、外部开工通知、合同参考、模具图片或其他依据冻结附件用途和内容 SHA-256；本人确认后才写入；客户分类由本轮操作者明确填写海信、海尔或其他并保存人工依据；若与项目客户规则冲突则阻止写入，不根据项目名称猜测；邮件和客户平台自动接收仍不是已具备能力；本系统保存人工接收或外部适配器提供后经本人确认的来源事实
- 验证证据：tests/test_bid_intake_tools.py 覆盖客户分类/合同/模具/承接上下文、确认前零写入、本轮附件约束、模具图片证据和权限隔离；真实邮件接收、正式合同登记衔接和业务人员验收仍未完成
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：query_bid_intake_context 将销售合同、承接决定和内部正式开工分开返回，并在合同存在但未见承接时提示合同不等于已承接；prepare_bid_intake_draft 同一版本可同时登记中标通知与外部开工附件，并冻结客户公司、联系人、客户模号、机型/物料、项目名、金额、收件人和匹配依据；合同参考附件只作为中标来源证据；正式销售合同仍通过独立合同工具、附件版本和 BPM 管理，财务付款条件不从中标草稿自动推断
- 验证证据：tests/test_bid_intake_tools.py 覆盖合同上下文、承接状态、多候选不自动决定、附件用途与不可变版本；财务确认金额/付款条件和真实业务审核仍未完整验收
- 验收状态：NOT_VERIFIED
...
- 实现证据：query_bid_intake_context 的 limitations 明确海尔等客户平台自动对接不作为已具备能力；当前只核对人工接收、维护、确认和依据；prepare_bid_intake_draft 可由人工登记客户外部订单号、外部开工日期和客户交期；后续仍由独立承接/拒单 BPM 确定内部生产、整套委外或拒单；有效拒单与有效正式开工分别终止同一中标草稿的普通补充路径，避免拒单后误入执行
- 验证证据：tests/test_bid_intake_tools.py 覆盖承接/拒单上下文、外部订单/开工资料补充和权限边界；海尔项目参与人员真实评估和客户平台适配尚未验收
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：query_bid_intake_context 不把客户规则固定为某一客户流程，按当前项目档案中的客户规则键和可见资料核对中标、合同、承接及开工事实；同一版本可同时绑定中标和外部开工附件；也可在稳定 case_id 下追加下一版补充后到的外部开工通知，状态变化不重复建单
- 验证证据：tests/test_bid_intake_tools.py 覆盖项目线索定位、缺少来源资料 gaps、同一 case 两版连续记录和重复内容防重；其他客户真实规则配置仍未验收
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：query_business_object_candidates 提供中标/承接前的候选项目匹配；query_quote_acceptance_context 可复用授权候选匹配定位项目并返回承接上下文；多候选返回候选列表并要求人工明确项目 ID，不虚填历史报价或历史模具；prepare_bid_intake_draft 经本人确认后创建稳定 bid_intake_case，并以 bid_intake_revision 追加保存客户公司、联系人、客户模号、机型/物料、金额、项目名称、我方收件人、报价匹配、历史模号和匹配依据；UNMATCHED 强制报价与历史模具字段为空；MATCHED 至少要求一个真实匹配对象，不能用占位值冒充匹配
- 验证证据：tests/test_business_matching.py 覆盖候选匹配与权限边界；tests/test_quote_tools.py 覆盖上下文多候选不自动决定；tests/test_bid_intake_tools.py 覆盖草稿创建、字段冻结、附件关联、同一 case 续版和中标上下文多候选不自动决定
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：query_bid_intake_context 仅在具备项目业务档案读取权限时返回内部模具或历史模具关联，并把承接决定、执行方式和拒单记录作为独立事实展示；prepare_bid_intake_draft 要求历史模号与 BACKUP/REFERENCE 关系成对人工填写；未匹配时两者必须留空，不自动猜测历史模具；承接/拒单仍进入 quote_acceptance 独立 BPM；岗位通知矩阵不由中标草稿越权代办
- 验证证据：tests/test_bid_intake_tools.py 覆盖模具关系权限隔离、备份/参考关系字段约束、承接/拒单上下文和多候选边界；岗位通知矩阵和真实角色评估仍未验收
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：bid_intake_case 为每个项目提供稳定唯一主记录；bid_intake_revision 采用连续版本和 previous_revision_id，承接前可创建、承接后可继续补充同一记录，内容与附件均不可 UPDATE/DELETE；bid_intake_lifecycle_link 把承接、拒单和内部正式开工材料挂回同一 case；在中标草稿晚于历史决定创建时会回填已有材料，后续新审批提交时即时建立链路；有效拒单后阻止继续补充执行草稿，拒单材料的原因和 BPM 依据仍保留在 quote_acceptance 记录中；项目启动链路新增独立中标接收阶段
- 验证证据：tests/test_bid_intake_tools.py 覆盖承接前建单、历史承接回填、同一 case 连续两版、重复内容防重和派生状态；tests/test_project_kickoff_lifecycle.py 覆盖报价生效后先进入中标接收再进入承接；真实拒单审批与正式开工全链路验收仍未完成
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：正式开工确认时写入不可变 InternalStartSnapshot，冻结外部订单、客户、客户模具号、机型/物料号、项目、内部模具、合同状态、开工日期、交期和中标接收版本；新模正式开工前必须已有 ERP 唯一内部模具号关联；已有模具设变必须复用中标接收记录指向的原内部模具号；销售合同与开工通知仍是独立业务对象；合同未到只要求登记预计到达日期，不阻塞满足客户开工条件的正式开工
- 验证证据：tests/test_start_tools.py 覆盖确认前卡片、确认后冻结快照、外部订单和内部模具号关联，以及合同缺失不阻塞正式开工；tests/test_project_kickoff_lifecycle.py 覆盖承接主线与合同并行建议，以及合同阶段与开工阶段不互相冒充；真实 ERP 模具建档仍待联调验收
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：无生效销售合同时 prepare_internal_start 强制登记 expected_contract_date，并随不可变开工材料保存；query_internal_start_readiness 和 query_finance_context 计算 PENDING、DUE_TODAY、OVERDUE、RECEIVED_DATE_MISSING、RECEIVED_ATTACHMENT_MISSING 或 RECEIVED 状态；逾期时生成只读被动预警投影，明确财务、项目负责人及业务/市场跟踪角色；合同登记要求实际到达日期并冻结本轮原件附件
- 验证证据：tests/test_start_tools.py 覆盖缺少预计日期阻断、逾期状态、财务和项目负责人提醒对象；tests/test_contract_tools.py 覆盖实际到达日期和合同附件随 BPM 材料保存；逾期提醒当前按技术方案第 11 章实现为用户查询触发的被动预警；真实通知渠道与业务/市场人员配置仍待现场验收
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：正式开工生效后向 FINANCE_OWNER 发送包含冻结开工材料的 Agent Core Outbox 交接，query_finance_context 同时返回开工通知、订单/合同材料和独立客户付款节点；prepare_mold_transfer_receipt 由财务按客户签收日期人工登记 CustomerDeliverySignature(move_type=MOLD_TRANSFER)，确认卡明确签收不等于质量验收；客户付款节点继续使用 PaymentStage 结构化维护，移模签收与 CustomerAcceptanceRecord 质量验收保持不同实体和不同结论
- 验证证据：tests/test_start_tools.py 覆盖正式开工财务角色交接和冻结材料；tests/test_finance_context_tools.py 覆盖结构化付款节点、财务移模签收确认卡、确认后写入及无质量验收副作用；真实 BPM 人员配置、客户签收单和财务岗位端到端操作仍待现场验收
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：prepare_contract_record 冻结合同号、签订日期、金额、交付日期、付款方式、付款节点和本轮合同原件，并把项目、客户、内部模具号、已确认客户订单和生效开工单写入不可变 ContractBusinessTerms 关联快照；query_contract_context 按项目或合同线索返回经权限过滤的销售合同、整套委外合同、付款节点、合同附件、业务条款和关联快照；不把审批材料伪装为 ERP 实时台账；合同明细必须同时具备对应合同查询工具和业务权限；上下文工具不上传凭证、不确认收付款
- 验证证据：tests/test_contract_tools.py 覆盖合同号定位、付款节点汇总、合同金额合计、签订/交付日期、付款方式、内部模具关联快照及审批附件；真实 ERP 财务对账仍待联调验收；tests/test_split_migrations.py 与 scripts/migrate.py check 覆盖 contract_business_terms 不可变 PostgreSQL 结构和双阶段迁移 head
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：prepare_contract_record 根据 Customer.rule_key 独立校验客户编号语义：海信使用客户项目号，海尔纸质合同使用合同号，海尔电子合同使用已确认客户订单号，其他客户保留人工核对依据；海尔电子合同订单号与 BidIntakeRevision.external_order_number 不一致时返回 CONTRACT_ORDER_MISMATCH；不同编号分别保存，不复用单一模糊字段；query_contract_context 可按合同号或项目线索定位当前可见项目；多候选要求用户指定项目 ID
- 验证证据：tests/test_contract_tools.py 覆盖合同号定位、多项目歧义、海尔电子合同订单号不匹配阻断和正确订单映射；海信真实文件字段及客户现场规则仍待联调验收
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：prepare_contract_record 只接受当前用户、当前会话、当前 Run 显式绑定的 PDF/图片/DOCX 合同原件；本人确认后才创建合同材料并提交 BPM；ContractAttachment 保存文档标识、第 1 版、电子/纸质扫描来源、上传人和上传时间；文件 SHA-256 及附件元数据冻结进审批快照，后续上传不能覆盖当前审批依据；同一项目、同一合同类型的有效合同附件按原件 SHA-256 防重；业务附件下载权限跟随合同读取授权
- 验证证据：tests/test_contract_tools.py 覆盖当前 Run 绑定、确认前不写入、确认后提交 BPM、附件版本/来源/快照、防重及合同范围读取权限；tests/test_split_migrations.py 覆盖 contract_attachment 表与四类文件/附件不可变触发器；真实业务主管/财务流程、后续合同版本及客户规则仍未验收
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：query_contract_context 的 cash_timing_analysis 按币种和已确认到期日汇总客户收款与整套委外付款，逐日计算流入、流出、净额和累计净额，并返回首个预计资金缺口；节点日期不完整时返回 DATES_INCOMPLETE；无生效销售合同但存在可见的生效报价承接时保留原收款条件文本并返回 UNSTRUCTURED_INFLOW_TERMS，不把文本强行解析为金额或日期；分析明确只读，不自动改变合同条款、不确认收付款、不替代采购或财务审批
- 验证证据：tests/test_contract_tools.py 覆盖销售合同与整套委外合同同时返回、权限隔离，以及供应商付款早于客户收款时首个预计缺口日期和金额；真实合同节点日期与财务资金安排仍待现场验收
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：backend/app/project_closure.py 区分终止/正常关闭并在终止生效后停止Agent本地未完成任务；backend/app/project_closure_tools.py 提供终止材料准备、人工确认和Agent BPM提交；ProjectClosureDetail保存终止依据、当前环节、完成工作和费用汇总；query_project_completion_context 把交付与客户验收、发票与客户回款、供应商结算、异常关闭、全过程归档和最终关闭保持为独立阶段，并按正常关闭或终止结算清单投影当前焦点；只读协调现有能力，不复制 ERP 台账
- 验证证据：tests/test_project_closure.py覆盖终止生效、任务停止和终止清单建立；内置浏览器验证合成终止建议、本人确认、审批通过及生效；tests/test_project_completion_lifecycle.py 覆盖协调器注册、正常关闭逐阶段满足、终止项目交付/验收明确不适用、财务未完成不被推断、阶段能力不泄漏及多项目候选不合并
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：终止清单保存未完成采购、在制品、供应商任务及客户/供应商结算处置；ERP事实要求原生引用和截至时间且不复制ERP台账；项目终止后使用专用待结算状态；query_project_completion_context 把交付与客户验收、发票与客户回款、供应商结算、异常关闭、全过程归档和最终关闭保持为独立阶段，并按正常关闭或终止结算清单投影当前焦点；只读协调现有能力，不复制 ERP 台账
- 验证证据：tests/test_project_closure.py覆盖ERP来源缺少引用时阻断及追加修订历史；真实ERP处置与结算联调待执行；tests/test_project_completion_lifecycle.py 覆盖协调器注册、正常关闭逐阶段满足、终止项目交付/验收明确不适用、财务未完成不被推断、阶段能力不泄漏及多项目候选不合并
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：终止关闭使用独立TERMINATION清单和SETTLEMENT_CLOSE决定；交付/验收允许记录原因后标为不适用；关闭结果为CLOSED_TERMINATION且历史不删除；query_project_completion_context 把交付与客户验收、发票与客户回款、供应商结算、异常关闭、全过程归档和最终关闭保持为独立阶段，并按正常关闭或终止结算清单投影当前焦点；只读协调现有能力，不复制 ERP 台账
- 验证证据：tests/test_project_closure.py覆盖终止结算关闭、不适用项和关闭后历史保护；专用PostgreSQL验收待执行；tests/test_project_completion_lifecycle.py 覆盖协调器注册、正常关闭逐阶段满足、终止项目交付/验收明确不适用、财务未完成不被推断、阶段能力不泄漏及多项目候选不合并
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：ContractDetail/PaymentStage 独立保存合同金额、节点名称、条件、金额、币种、比例、触发事件、触发日期、账期天数、预计到期日、确认依据、特殊标记和确认状态；query_finance_context 将销售合同收款节点作为 customer_receivable_nodes 和 customer_receivable_schedule 返回，明确节点条件需以合同和财务确认为准；prepare_customer_receivable_schedule 通过会话确认卡维护既有生效销售合同节点的结构化账期；本人确认前不写库，确认后保留审计，不修改合同金额或登记实际回款
- 验证证据：tests/test_finance_context_tools.py 覆盖 DFM 认证节点、比例/金额、触发事件、账期、到期日、确认依据、特殊标记以及确认卡写入边界
- 验收状态：NOT_VERIFIED
...
- 最新口径：完整保留；具体既有动作复用不抵消本条需求。
- 实现证据：query_finance_context 按合同实际 PaymentStage 条件返回节点，不把 T0、DFM、试模、签收、验收或账期示例套用为统一规则；工具 warnings 标识未确认条件和未接入客户实际回款台账，区分节点条件、提醒和实际收款；customer_receivable_schedule 只按财务确认的结构化触发日期、账期/到期日和实际回款汇总计算未触发、未到期、到期未收、逾期未收、部分回款和已回款；自由文本缺字段时保持待配置，不解析示例天数；到期或逾期节点输出结构化 reminder、未收金额和逾期天数；提醒不执行自动催款，特殊标记只能经财务确认卡保存
- 验证证据：tests/test_finance_context_tools.py 覆盖自由文本不推断日期、结构化节点结合分次实际回款计算部分回款逾期、提醒清单和未收金额；真实 ERP 回款来源联调及正式财务验收仍待完成
- 验收状态：NOT_VERIFIED
