...
其中，`query_design_route_context` 是本地只读上下文能力，`prepare_design_order_approval` 负责把 ERP 只读证据冻结进 Agent BPM；其余 ERP 资料能力通过随 Mold 业务包配置的 ERP 设计 MCP 调用现有 ERP 接口。这里的“工具”是可被模型调用的操作，“技能”是引导模型选择工具、安排步骤和控制确认的业务流程。
...
- 不提供独立的“ERP 设计管理”侧栏入口或管理站点。
- 用户在对话中查询设计订单时，`erp_design_query_orders` 的结果生成“ERP 设计订单已就绪”卡片；点击“查看订单”在当前对话上方打开只读订单表格弹窗。
- 设计订单也可随审批材料和附件上下文查看；图纸版本、材质密度和厂内标准件不创建专用页面。它们分别通过现有 ERP 工具读取 `D:\work2\management-system` 使用的数据库或标准件文件目录，并在对话中返回证据。
- 设计订单弹窗不复制业务数据、不绕过 Agent 权限，也不直接代替对话执行审批、删除或修改；涉及写入仍由对应工具按确认流程执行。
...
| 1A | `prepare_design_order_approval` | 读取指定 ERP 设计订单，准备冻结证据、当前对话附件和 Agent BPM 审批；不调用 ERP 审批。 |
...
| `erp_design_order_approval`<br>ERP 设计订单提交 Agent 审批 | “把 ERP 设计订单 128 提交设计审批”“新模清单走 Agent 审批” | 项目及当前版本、ERP 订单 ID、新模/改模类型、图纸版本、复核人、Agent BPM 模板和当前对话附件 | 固定接口读取 ERP 订单并生成确认卡；用户确认时再次核对 ERP 哈希，然后创建本地设计材料、锁定附件与快照并提交 Agent BPM。全过程不调用 ERP 设计审批。 |
...
| `erp_design_workspace_review`<br>ERP 设计资料核对 | “查 ERP 设计订单”“查看设计订单表格”“设计与物料清单”“BOM 采购进度” | 模号、订单号、BOM 关键词或筛选条件 | 普通订单查看只调用一次订单列表查询；只有明确要求物料/BOM 信息时才追加对应 BOM 查询。 |
...
### 1. MoldPilot 本地设计上下文与 Agent 审批（2 个）
...
| `prepare_design_order_approval` | 把指定 ERP 设计订单作为新模/改模审批材料 | 项目 ID/版本、ERP 订单 ID、设计类型、图纸版本、复核人、Agent BPM 模板、当前 Run 的附件 ID | 只读获取 ERP 订单并锁定来源、版本、时点、哈希、摘要、原始快照和附件。确认时再次读取 ERP；哈希变化则拒绝。确认成功只提交 Agent BPM，不调用 ERP 审批。 |
